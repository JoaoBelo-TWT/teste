...
export default async function SocialStatisticsPage({
  params,
  ...
  const currentActivityPagination = getCurrentPagination({ pageIndex: activityPageIndex, pageSize: activityPageSize });

  const [t, channelPerformanceDeepDive, channelPerformanceDeepDiveActivityDetails] = await Promise.all([
    getTranslations(),
    fetchChannelPerformanceDeepDiveOverview(params.dashboardId, ChannelFiltersDashboardEnum.Social, timeframe),
    fetchChannelPerformanceDeepDiveActivityDetails(
      params.dashboardId,
      ChannelFiltersDashboardEnum.Social,
      timeframe,
      currentActivityPagination.query
    )
  ]);
  ...
  return (
    <>
      <BaseCard
        ...
        headerProps={{
          title: t('general.channels.social')
          // children: <FilterDropdown setFilter={setFilter} filter={filter} />
        }}
        topContent={
          <Breadcrumbs
            items={[
      ...
      <DetailsTable
        variant={DetailPage.SOCIAL}
        pagination={currentActivityPagination.info}
