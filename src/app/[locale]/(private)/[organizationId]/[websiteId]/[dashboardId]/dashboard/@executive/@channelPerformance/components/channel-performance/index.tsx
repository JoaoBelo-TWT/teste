...
export default function ChannelPerformance() {
  ...
  const getFunnelsFiltersOptions = useCallback(() => {
    if (!data?.dashboardChannelPerformance.channelsPerformance) return [];

    return data?.dashboardChannelPerformance.channelsPerformance.map((c) => ({
      value: c.stageId,
      label: c.stageName
    }));
  }, [data?.dashboardChannelPerformance]);

  const selectedFunnel = useMemo(
    () =>
      data?.dashboardChannelPerformance?.channelsPerformance?.find((c) => c.stageId === filters.channel) ??
      data?.dashboardChannelPerformance?.channelsPerformance?.find((c) => c),
    [data?.dashboardChannelPerformance, filters.channel]
  );
  ...
  if (!data?.dashboardChannelPerformance?.isSetup) {
    return (
  ...
  if (!data?.dashboardChannelPerformance.hasEvents) {
    return (
      <ChannelPerformanceEmpty
        variant="no-data"
        dashboardId={dashboardId}
