...
import { useQueryWebsitePerformance } from '@/lib/react-query/dashboard/executive/query-website-performance';
...
export default function WebSiteActivity() {
    ...
    () => (date: string) => {
      switch (filters.timeframe) {
        ...
        case DashboardTimeframe.ThisWeek:
          return dayjs(date).utc().format('MMM DD');
  ...
  const chartData = useMemo(
    () =>
      data.dashboardWebsiteActivity?.traffic.map((traffic, index, arr) => ({
        value: traffic.pageViews,
        date: getChartDate(traffic.date),
        originalDate: traffic.date,
        hideDot: index !== lastDataPointIndexWithValue,
        previousValue: index > 0 ? arr[index - 1].pageViews : undefined
      })) ?? [],
