...
import { CreateActivityGoal } from './action';
import classes from './index.module.css';
import { ActivityGoalFormProps } from './types';

export function CreateActivityGoalForm({ dashboardId, customerFunnelStages }: Readonly<ActivityGoalFormProps>) {
  ...
  const {
    ...
    defaultValues: {
      numberOfEvents: 1,
      customerFunnelStageId: undefined
    }
  });

  const formAction: () => Promise<void> = handleSubmit(async (formData) => {
    const response = await CreateActivityGoal({ ...formData, dashboardId });
  ...
  return (
    <form action={formAction} className={classes['create-activity-goal']}>
      <HeroControl
        data-autofocus
      ...
      <div className={classes['create-activity-goal-form__submit-container']}>
        <ButtonForm
          variant="outline"
